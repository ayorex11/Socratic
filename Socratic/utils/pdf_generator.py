...
class PDFGenerator:
    """
    Generates downloadable PDF reports with improved formatting
    """
    
    @staticmethod
    def generate_report(processing_result, output_filename):
        """
        Generate a PDF report with summary and Q&A
        """
        try:
            ...
            file_path = os.path.join(pdf_dir, f"{output_filename}.pdf")
            ...
            formatted_summary = PDFGenerator.parse_markdown_text(summary_text)
            ...
            return f"pdf_reports/{output_filename}.pdf"
    ...
    def parse_markdown_text(text):
        ...
        while i < len(lines):
            ...
            # Handle headings (###)
            if line.startswith('### '):
                heading_text = line[4:].strip()  # Remove '### '
            ...
            if line.startswith('* ') or line.startswith('- '):
                bullet_text = line[2:].strip()  # Remove '* ' or '- '
            ...
            if line:
                ...
                j = i + 1
                while j < len(lines) and lines[j].strip() and not any(lines[j].strip().startswith(pattern) for pattern in ['###', '* ', '- ', '|', '']):
...
class AdvancedPDFGenerator(PDFGenerator):
    ...
    def generate_report(processing_result, output_filename):
        """
        Generate a premium PDF with enhanced formatting
        """
        try:
            pdf_dir = os.path.join(settings.MEDIA_ROOT, 'pdf_reports')
            os.makedirs(pdf_dir, exist_ok=True)
            ...
            # Context with better styling
            context_bg = '#e8f6f3' if processing_result.used_past_questions else '#fef9e7'
            ...
            summary_text = processing_result.summary
            formatted_summary = AdvancedPDFGenerator.parse_markdown_text_premium(summary_text)
    ...
    @staticmethod
    def parse_markdown_text_premium(text):
        """
        Enhanced markdown parsing for premium reports
        """
        elements = PDFGenerator.parse_markdown_text(text)
        return elements