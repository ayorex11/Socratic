...
class Transaction(models.Model):
    ...
    pricing = models.ForeignKey(Pricing, on_delete=models.CASCADE, blank=False, null=False)
    date_created = models.DateTimeField(auto_now_add=True)
    date_completed = models.DateTimeField(blank=False)
    completed = models.BooleanField(default=False)
    reference = models.CharField(max_length=50, blank=False, null=False)

    def __str__(self):
        return self.user + " " + self.amount_paid
